...
const filterStringsWithVowels = (arr) => {
  ...
  let answer = arr.filter( word => {
    let wordSplit = word.split('');
    let decision = false;
    for (var vowel of vowels) {
      if (wordSplit.includes(vowel)) { decision = true; }
    }
    return decision;
  });
  return answer;
...
const notInFirstArray = (forbiddenValues, arr) => {
  let answer = arr.filter( num => {
    let decision = true;
    for (var badValue of forbiddenValues) {
      if (num === badValue) { decision = false; }
    }
    return decision;
  });
  return answer;
...
const getStatName = (arr, minBaseStat) => {
  let filteredArr = arr.filter( snorlax => snorlax.baseStat > minBaseStat);
  return filteredArr.map ( snorlax => snorlax.stat.name);
};
...
Write a git function named evenOddNumericValues that, given an array as input, uses filter to remove any non-numeric values, then uses map to generate a new array containing the string 'even' or 'odd', depending on the original value.
...
const evenOddNumericValues = (arr) => {
  ...
  return filteredArr.map( element => element%2===0 ? 'even' : 'odd');
};
