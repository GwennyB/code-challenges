...
const houseSize = (arr) => {
  ...
  arr.forEach(element => {
    ...
    let houseStat = Object.values(element);
    if(houseStat[1]) {
      thisHouse.members++;
    }
    thisHouse.members += houseStat[2].length;
    sizes.push(thisHouse);
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(element => {
    ...
    let houseStat = Object.values(element);
    if(houseStat[1] && !deceasedSpouses.includes(houseStat[1])) {
      thisHouse.members++;
    }
    thisHouse.members += houseStat[2].length;
    survivors.push(thisHouse);
  });
  return survivors;
};
